...
1. **普通沟通训练**：进入“领导质问进度”等普通沟通模块，与 LLM 模拟角色完成最多五轮对话。
2. **女性 PUA 应对训练**：从职场或家庭的 10 个独立模块中选择场景和容易/中等/困难等级，在受控压力话术下练习识别、澄清、立场、边界和下一步。
3. **真实经历训练**：用户讲述一段真实经历，AI 逐步补问缺失信息，生成可确认的场景卡、角色卡和学习目标，再进入同一套对练与复盘流程。
...
- 角色对话使用服务端意图账本记录已用动作、已问核心意图和已承接事实槽；下一轮只允许从尚未使用的动作白名单中选择。
- 轻量本地 RAG 从角色卡和相近场景检索少量口语节奏参考，模型只能借鉴表达方式，不得复制参考事实或整句。
- 三份 PUA 材料逐句标注来源、领域、场景类型、压力手法、目标人群和 1–3 级语言强度。
- RAG 使用双通道：普通沟通把 PUA 语料作为禁区反例；PUA 应对模块把当前场景和难度允许的语料作为受控角色材料。
- 前端场景下拉框可直接选择 1 个普通模块或 10 个 PUA 独立模块。
- 自动 API 文档：`/docs`。
...
角色模拟主流程会只读使用其中 `scenes.json` 的短对话作为普通沟通表达参考；模式分析、策略建议、语气路由和评测数据仍与网页运行时分离。

PUA RAG 采用“先选模块、再过滤、后排序”的方式，不会把所有材料投入同一次相似度搜索：

```text
training_mode
  ├─ ordinary → expression_references + pressure_anti_examples
  └─ pua_response
       → module_id
       → domain + scenario_types
       → severity <= difficulty
       → 排除历史近似台词
       → controlled_pressure_examples（最多 3 条）
       → 当前模块角色卡 + 意图账本 + LLM 最终措辞
```
...
1. 对 68 条自动标注结果进行人工复核，尤其检查一句话多标签和难度边界。
2. 为 10 个 PUA 模块分别增加固定用户回答集，评测角色强度、模块越界、重复施压和边界后的松动行为。
3. 为智谱对话增加流式输出或更细的加载状态，降低每轮等待感。
