const I18N = {
  ...
  "guide-copy": "\u5f00\u5c4f\u3001\u5f55\u97f3\u590d\u76d8\u3001\u7ecf\u5178\u5bf9\u7ec3\u3001\u66b4\u51fb\u53d1\u6cc4\u4e0e\u8bbe\u7f6e\u7684\u5b8c\u6574\u53ef\u70b9\u51fb\u6d41\u7a0b\u3002",
  "guide-splash": "\u5f00\u5c4f\u4e0e\u52a0\u8f7d",
  "guide-home": "\u9996\u9875\u4e0e\u5f85\u5206\u6790\u5f55\u97f3",
  ...
  "direct-analyze": "\u76f4\u63a5\u5206\u6790",
  "privacy": "\ud83d\udd12 \u5b58\u50a8\u540e\u4f1a\u51fa\u73b0\u5728\u9996\u9875\u4e0b\u65b9\uff1b\u53ea\u6709\u4e3b\u52a8\u70b9\u51fb\u65f6\u624d\u5f00\u59cb AI \u5206\u6790\u3002",
...
const seedRecordings = [{ id: "family-dinner", title: COPY.seedTitle, meta: COPY.seedMeta }];
const TAB_ORDER = { splash: -1, home: 0, recording: 0, analysis: 0, practice: 1, levels: 1, custom: 1, drill: 1, vent: 2, settings: 3 };
      ...
      (recording) => `
        <article class="saved-card">
          ...
          <div class="saved-bottom">
            ...
            <button class="figma-button primary" data-action="analyze">${COPY.analyzeNow}</button>
          </div>
        </article>
      `,
    )
...
function showScreen(id, { scrollInbox = false, direction } = {}) {
  ...
  if (prev && prev !== next) {
    if (dir === "splash") prev.classList.add("splash-out");
    else prev.classList.add(dir === "back" ? "leaving-right" : "leaving-left");
  ...
  else if (dir !== "splash") next.classList.add("enter-from-right");
  next.offsetHeight;
...
guides.forEach((guide) => {
  guide.addEventListener("click", () => {
    if (guide.dataset.jump === "splash") playSplash();
    else showScreen(guide.dataset.jump);
  });
});
...
function openLevels(scene) {
  ...
  levelsList.innerHTML = pack.items
    .map(
      (item) => `
...
function hitVent(power = 1) {
  const now = Date.now();
  combo = now - lastHit < 780 ? combo + power : power;
  lastHit = now;
  ventCounter.textContent = COPY.combo.replace("{n}", String(combo));
...
function playSplash() {
  showScreen("splash", { direction: "splash" });
  const frames = [...splashLoader.querySelectorAll(".loader-frame")];
  frames.forEach((frame, index) => frame.classList.toggle("is-on", index === 0));
  [0, 720, 1480, 2140].forEach((delay, index) => {
    window.setTimeout(() => {
      frames.forEach((frame, frameIndex) => frame.classList.toggle("is-on", frameIndex === index));
    }, delay);
  });
  window.setTimeout(() => {
    if (currentScreen === "splash") showScreen("home", { direction: "splash" });
  }, 2680);
}
...
document.querySelector("#coach-form").addEventListener("submit", (event) => {
  ...
  response.textContent = COPY.aiResponse;
  event.currentTarget.hidden = true;
  input.value = "";
  requestAnimationFrame(() => response.scrollIntoView({ behavior: "smooth", block: "center" }));
});
...
ventCounter.textContent = COPY.combo.replace("{n}", "0");
playSplash();
